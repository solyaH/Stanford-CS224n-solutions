...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        # prj output of last h_state of encode (R^2h) to R^h
        ...
        # use after combined attention output and h_decode
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        # 1
        X = self.model_embeddings.source(source_padded)  # (src_len, b, h*2)
        ...
        init_decoder_hidden = self.h_projection(last_hidden)  # W_h(h) -> (b, h*2)*(2*h, h) -> (b, h)

        last_cell = torch.cat((last_cell[0], last_cell[1]), 1)  # (2, b, h) -> (b, h*2)
        init_decoder_cell = self.c_projection(last_cell)  # W_c*c
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        # 1
        enc_hiddens_proj = self.att_projection(enc_hiddens)  # W_att(h_enc) -> (b, h*2)*(2*h, h) -> (b, h)
        ...
        # 4
        combined_outputs = torch.stack(combined_outputs)  # list(tgt_len) -> (tgt_len, b, h)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        @param enc_hiddens_proj (Tensor): Encoder hidden states Tensor, projected from (h * 2) to h. Tensor is with shape
                                    (b, src_len, h),
                                    where b = batch size, src_len = maximum source length, h = hidden size.
