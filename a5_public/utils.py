...
def pad_sents_char(sents, char_pad_token):
    ...
    # sents = pad_sents(sents, list())
    max_sentence_length = max([len(x) for x in sents])
    sents = [x + [[char_pad_token] * max_word_length] * (max_sentence_length - len(x)) for x in sents]
    sents_padded = [[w + [char_pad_token] * (max_word_length - len(w)) for w in sent] for sent in sents]
    ### END YOUR CODE
