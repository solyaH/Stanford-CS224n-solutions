...
def question_1h_sanity_check(highway_model):
    ...
    assert (highway_model.c_projection.in_features == NUM_FILTER), \
        "Input dimension for W_proj is incorrect:\n " \
        "it should be {} but is: {}".format(NUM_FILTER, highway_model.c_projection.in_features)
    assert (highway_model.c_projection.out_features == NUM_FILTER), \
        "Output dimension for W_proj is incorrect:\n " \
        "it should be {} but is: {}".format(NUM_FILTER, highway_model.c_projection.out_features)
    assert (highway_model.g_projection.out_features == NUM_FILTER), \
        "Input dimension for W_gate is incorrect:\n " \
        "it should be {} but is: {}".format(NUM_FILTER, highway_model.g_projection.out_features)
    assert (highway_model.g_projection.out_features == NUM_FILTER), \
        "Output dimension for W_gateis incorrect:\n " \
        "it should be {} but is: {}".format(NUM_FILTER, highway_model.g_projection.out_features)

    x_conv_out = torch.randn((BATCH_SIZE, NUM_FILTER))
    x_highway = highway_model(x_conv_out)
...
def question_1i_sanity_check(cnn_model):
    ...
    assert (cnn_model.conv1d.in_channels == EMBED_SIZE), \
        "Input dimension for Conv1d is incorrect:\n " \
        "it should be {} but is: {}".format(EMBED_SIZE, cnn_model.conv1d.in_channels)
    assert (cnn_model.conv1d.out_channels == NUM_FILTER), \
        "Output dimension for Conv1d is incorrect:\n " \
        "it should be {} but is: {}".format(NUM_FILTER, cnn_model.conv1d.out_channels)
    # assert (cnn_model.conv1d.kernel_size[0] == KERNEl_SIZE), \
    #     "Kernel size dimension for Conv1d is incorrect:\n " \
    #     "it should be {} but is: {}".format(KERNEl_SIZE, cnn_model.conv1d.kernel_size[0])
    assert (cnn_model.pooling.kernel_size == MAX_WORD_LEN-KERNEl_SIZE+1), \
        "Kernel size dimension for Conv1d is incorrect:\n " \
        "it should be {} but is: {}".format(MAX_WORD_LEN-KERNEl_SIZE+1, cnn_model.pooling.kernel_size)

    x_reshaped = torch.randn((BATCH_SIZE, EMBED_SIZE, MAX_WORD_LEN))
    x_out = torch.zeros((BATCH_SIZE, NUM_FILTER))
    x_cnn = cnn_model(x_reshaped)
    assert x_out.size() == x_cnn.size(),\
        "Output size should be: {}\n but is {}\n".format(x_out.size(), x_cnn.size())
...
def main():
    ...
    assert (torch.__version__ == "1.0.0"),\
        ...
        "You have {} and you should have version 1.0.0".format(torch.__version__)
    ...
    highway_model = Highway(embed_word_size=NUM_FILTER)

    cnn_model = CNN(embed_char_size=EMBED_SIZE,
                    max_word_length=MAX_WORD_LEN,
                    filters_size=NUM_FILTER,
    ...
    if args['1e']:
        ...
        question_1h_sanity_check(highway_model)
    elif args['1i']:
        question_1i_sanity_check(cnn_model)
    elif args['2a']:
