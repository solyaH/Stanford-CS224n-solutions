...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        ...
        for partial_parse, next_transition in zip(minibatch, next_transitions):
            partial_parse.parse([next_transition])
            if len(partial_parse.buffer) == 0 and len(partial_parse.stack) == 1:
                unfinished_parses.remove(partial_parse)

    dependencies = [el.dependencies for el in partial_parses]
    ### END YOUR CODE
